...
Spectrum::Spectrum(const engine::ui::components::Layout &layout, int bars, float gap) : UiNode(layout), m_bars(bars), m_gap(gap), m_target_levels(bars, 0.0f), m_displayed_levels(bars, 0.0f) {
}
...
void Spectrum::update(double delta_time) {
    for (std::size_t i = 0; i < m_bars; ++i) {
        ...
        const float duration = target > m_displayed_levels[i] ? default_attack_speed : default_release_speed;
...
void Spectrum::update_by_audio(audio::AudioBuffer &audio_buffer, double track_time) {
    ...
    const float highest_frequency = std::min(maximum_frequency, nyquist);
    ...
    const float frequency_ratio = highest_frequency / minimum_frequency;
    ...
    for (int bar = 0; bar < m_bars; ++bar) {
        ...
        const float lower_frequency = minimum_frequency * std::pow(frequency_ratio, start_t);
        // NOTE: Check with max freq
        const float upper_frequency = minimum_frequency * std::pow(frequency_ratio, end_t);
        ...
        m_target_levels[bar] = std::clamp((decibels - minimum_decibels) / (maximum_decibels - minimum_decibels), 0.0f, 1.0f);
    }
}

void Spectrum::submit(graphics::renderer::Renderer &renderer) const {
    ...
    const std::filesystem::path texture_path = "resources/core/textures/colours/white.png";

    const int texture_id = renderer.texture_id(texture_path);

    const graphics::texture::Region &region = renderer.texture(texture_path).get_region(texture_path);

    const float u = (region.uv.u0 + region.uv.u1) * 0.5f;
    const float v = (region.uv.v0 + region.uv.v1) * 0.5f;

    const graphics::effect::EffectPtr effect = renderer.bloom_effect(m_layout.bloom.intensity);
